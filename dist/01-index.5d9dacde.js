...
document.addEventListener('DOMContentLoaded', async ()=>{
    ...
    const displayMovies = async (movies)=>{
        for (const movie of movies){
            // Fetch movie details
            const details = await (0, _apiJs.fetchMovieDetails)(movie.imdbID);
    ...
    const loadSpecificMovies = async ()=>{
        try {
            ...
            const movies = [
                ...(await (0, _apiJs.fetchMovies)('Transformers', 1)).slice(0, 1),
                ...(await (0, _apiJs.fetchMovies)('The Count of Monte Cristo', 1)).filter((movie)=>movie.Year === '2024'),
                ...(await (0, _apiJs.fetchMovies)('Conjuring', 1)).slice(0, 1) // 1st Conjuring movie
            ];
            if (movies.length > 0) await displayMovies(movies);
            else console.error('No specific movies found.');
        } catch (error) {
    ...
    const loadMoreMovies2024 = async ()=>{
        try {
            const movies2024 = (await (0, _apiJs.fetchMovies)('2024', currentPage)).filter((movie)=>movie.Year === '2024');
            if (movies2024.length > 0) await displayMovies(movies2024);
            else {
...
},{"./shared/api.js":"5SM8C"}],"5SM8C":[function(require,module,exports,__globalThis) {
...
const fetchMovies = async (query, page = 1)=>{
    try {
        ...
        if (data.Response === "True") return data.Search; // Returns the found movies
        else {
...
const fetchMovieDetails = async (id)=>{
    try {
        ...
        const data = await response.json();
        if (data.Response === "True") return data; // Returns the movie details
        else {
...
//# sourceMappingURL=01-index.5d9dacde.js.map
